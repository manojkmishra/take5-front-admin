   ...
   <v-data-table  :headers="headers" :items="sawflags"  class="elevation-1" >
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>JOB TYPES</v-toolbar-title>
        <v-divider class="mx-4" inset vertical></v-divider>
        <v-spacer></v-spacer>
        <!--------------modal------------------->
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark rounded class="mb-2" v-on="on">New Type</v-btn>
          </template>
          <!----popup---------------->
          <v-card>
              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="6" md="6">
                    <v-text-field v-model="editedItem.type" label="Name"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="6">
 ...
 import Vue from 'vue';
    ...
    {  data: () => ({
      ...
      headers: [
                        ...
                        { text: "NAME", align: "left", sortable: true, value: "type" },
    ...
    methods: { 
      ...
      {  //console.log('save-item=',item);
        if (this.editedIndex > -1) //save clicked when editing
                  {  console.log('edit',this.editedItem)
                    //edit api here
                    this.$store.dispatch('editjobtypes', this.editedItem) 
                        .then((response) => {})     .catch((error) => {});
                    } 
           //--------save clicked when adding new
        else {  console.log('add-item',this.editedItem)
                    //add new api here
                    this.$store.dispatch('addjobtypes', this.editedItem) 
                      .then((response) => {})     .catch((error) => {});
            }
                this.close()
        },
      ...
      remove() { console.log('remove()- editedIndex', this.editedIndex)
                  // delete api here
                  this.$store.dispatch('deljobtypes', this.editedItem)  
                                .then((response) => {})
                                .catch((error) => {});
